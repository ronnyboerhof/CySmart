...
public class OTAFilesListingActivity extends Activity {
    ...
    private final ArrayList<OTAFileModel> arrayListFiles = new ArrayList<OTAFileModel>();
    ...
    private OTAFileListAdapter firmwareAdapter;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        firmwareAdapter = new OTAFileListAdapter(this,
                arrayListFiles, mFilesCount);
        fileListView.setAdapter(firmwareAdapter);
        ...
        fileListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            public void onItemClick(AdapterView<?> parent, View view,
                ...
                OTAFileModel model = arrayListFiles.get(position);
                model.setSelected(!model.isSelected());
                for (int i = 0; i < arrayListFiles.size(); i++) {
                    if (position != i) {
                        arrayListFiles.get(i).setSelected(false);
                    }
                }
        ...
        mUpgrade.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (mFilesCount == OTAFirmwareUpgradeFragment.mApplicationAndStackSeparate) {
                    for (int count = 0; count < arrayListFiles.size(); count++) {
                        if (arrayListFiles.get(count).isSelected()) {
                            mArrayListPaths.add(1, arrayListFiles.get(count).getFilePath());
                            mArrayListFileNames.add(1, arrayListFiles.get(count).getFileName());
                        }
                    }
                } else {
                    for (int count = 0; count < arrayListFiles.size(); count++) {
                        if (arrayListFiles.get(count).isSelected()) {
                            mArrayListPaths.add(0, arrayListFiles.get(count).getFilePath());
                            mArrayListFileNames.add(0, arrayListFiles.get(count).getFileName());
                        }
                    }
                }
                ...
                if (mFilesCount == OTAFirmwareUpgradeFragment.mApplicationAndStackSeparate) {
                    if (mArrayListPaths.size() == 2) {
                        ...
                        alertFileSelection(getResources().getString(R.string.toast_file_applicationstacksep));
                    }
                    ...
                    if (mFilesCount != OTAFirmwareUpgradeFragment.mApplicationAndStackCombined) {
                        alertFileSelection(getResources().getString(R.string.toast_file_application));
                    } else {
                        alertFileSelection(getResources().getString(R.string.toast_file_applicationstackcomb));
                    }
            }
        }
    }
    ...
    mNext.setOnClickListener(new View.OnClickListener() {
        ...
        for (int count = 0; count < arrayListFiles.size(); count++) {
            if (arrayListFiles.get(count).isSelected()) {
                mArrayListPaths.add(0, arrayListFiles.get(count).getFilePath());
                mArrayListFileNames.add(0, arrayListFiles.get(count).getFileName());
                Toast.makeText(OTAFilesListingActivity.this, getResources().getString((R.string.ota_toast_stack_file_selected)),
                        Toast.LENGTH_SHORT).show();
                mHeading.setText(getResources().getString((R.string.ota_app_file)));
                arrayListFiles.remove(count);
                firmwareAdapter.addFiles(arrayListFiles);
                firmwareAdapter.notifyDataSetChanged();
                mUpgrade.setVisibility(View.VISIBLE);
                mNext.setVisibility(View.GONE);
            }
